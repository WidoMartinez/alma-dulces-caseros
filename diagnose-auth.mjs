...
try {
  ...
  allUsers.forEach(u => {
    ...
    console.log(
      `    Password: ${u.password ? "✓ Configurada" : "✗ Sin configurar"}`
    );
  });
  ...
  for (const test of testCases) {
    ...
    console.log(
      `  ${isValid ? "✅" : "❌"} Contraseña "${test.password}": ${isValid ? "CORRECTA" : "INCORRECTA"}`
    );
    ...
    if (!isValid) {
      ...
      console.log(
        `  ✅ Hash actualizado y verificado: ${testNew ? "OK" : "ERROR"}`
      );
    }
  }
